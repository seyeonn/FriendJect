        ...
        <b-col sm="5">
          <div class="list-group" >
            ...
            <div class="list-group-item" v-for="doc in documetns" :key="doc.id">
              ...
              <div>
                <h4 class="list-group-item-heading"><b-icon icon="exclamation-circle"></b-icon> 
                  <b-button variant="light" @click="getDocument(doc.id, $event)"> {{ doc.title }}</b-button>
                </h4>
                <h5><b-icon icon="calendar4-event"></b-icon> {{ doc.meeting_time | yyyyMMddHHmm }}</h5>
              </div>
            </div> 
          </div>
        </b-col>
          ...
          <div class="form" v-if="postable">
            ...
            <div>
              <div>
                ...
                <b-textarea rows="8" placeholder="content" v-model="content"></b-textarea>
                <button class="button basic outline" @click="postDocument">Submit</button>
              </div>
            </div>
          </div>
          ...
          <div class="form" v-if="editable">
            ...
            <div class="panel-body">
              <div>
                ...
                <button class="button basic outline" @click="updateDocument(document.id, $event)">수정완료</button>
              </div>
            </div>
          </div>
          ...
          <div v-if="document && showDetail">
            ...
            <div>
              <b-card :title="document.title">
                ...
                <template #footer>
                  <small class="text-muted"><time>{{ document.meeting_time | yyyyMMddHHmm }}</time></small>
                </template>
              </b-card>
            </div>
              <div>
                <button class="button basic outline" @click="showUpdateForm">Update</button>
                <button class="button basic outline" @click="deleteDocument(document.id, $event)">Delete</button>
              </div>
            </div>
